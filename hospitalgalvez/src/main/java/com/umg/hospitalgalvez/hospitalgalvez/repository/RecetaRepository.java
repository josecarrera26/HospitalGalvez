...
public interface RecetaRepository extends JpaRepository<Receta, Long> {
    @Query("select r.id_receta, c.id_cita, r.fechaCreacion, p.nombre , m.nombre_medico " +
            ...
            +
            " join Paciente p on p.id_paciente = c.paciente.id_paciente where r.id_receta = :id_receta")
    List<Object[]> findRecetasByMedico(@Param("id_receta") Long id_receta);

    @Query("select r.id_receta, c.id_cita, r.fechaCreacion, p.nombre , m.nombre_medico " +
            " from Receta r join Cita c on c.id_cita = r.cita.id_cita join Medico m on m.id_medico  = c.medico.id_medico"
            +
            " join Paciente p on p.id_paciente = c.paciente.id_paciente")
    List<Object[]> findAllRecetas();
